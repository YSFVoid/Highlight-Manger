...
class VoteSubmissionView(discord.ui.View):
    def __init__(self, match_service: MatchService, guild: discord.Guild, match: MatchRecord) -> None:
        ...
        if match.mode.team_size > 1:
            self.add_item(PlayerChoiceSelect(guild, match.team1_player_ids, "team_1_mvp", "Team 1 MVP candidate"))
            self.add_item(PlayerChoiceSelect(guild, match.team2_player_ids, "team_2_mvp", "Team 2 MVP candidate"))
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if view.match.mode.team_size > 1:
            if view.team1_mvp_id is None or view.team2_mvp_id is None:
                return await interaction.response.send_message("Select one MVP candidate from each team first.", ephemeral=True)
