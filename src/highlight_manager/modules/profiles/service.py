...
class ProfileService:
    ...
    async def set_queue_activity_for_players(self, repository: ProfileRepository, player_ids: list[int], queue_id: UUID) -> None:
        await repository.set_activity_for_players(player_ids, activity_kind=ActivityKind.QUEUE, queue_id=queue_id)

    async def set_match_activity_for_players(self, repository: ProfileRepository, player_ids: list[int], match_id: UUID) -> None:
        await repository.set_activity_for_players(player_ids, activity_kind=ActivityKind.MATCH, match_id=match_id)

    async def set_tournament_activity_for_players(
        self,
        repository: ProfileRepository,
        player_ids: list[int],
        tournament_id: UUID,
    ) -> None:
        await repository.set_activity_for_players(
            player_ids,
            activity_kind=ActivityKind.TOURNAMENT,
            tournament_id=tournament_id,
        )

    async def clear_activity(self, repository: ProfileRepository, player_ids: Iterable[int]) -> None:
        await repository.set_activity_for_players(list(player_ids), activity_kind=ActivityKind.IDLE)
