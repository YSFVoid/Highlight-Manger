...
class SetupService:
    ...
    async def run(self, guild: discord.Guild, *, prefix: str | None, repair: bool = False) -> SetupRunResult:
        ...
        apostado_play_channel, created = await self._ensure_text_channel(
            guild,
            configured_id=config.apostado_play_channel_id,
            name="apostado-play",
            reason="Highlight Manager automatic setup",
        )
        created_resources.append(f"Apostado Play Room: {apostado_play_channel.mention}") if created else reused_resources.append(f"Apostado Play Room: {apostado_play_channel.mention}")
        setup_resource_ids["apostado_play_channel_id"] = apostado_play_channel.id

        highlight_play_channel, created = await self._ensure_text_channel(
            guild,
            configured_id=config.highlight_play_channel_id,
            name="highlight-play",
            reason="Highlight Manager automatic setup",
        )
        created_resources.append(f"Highlight Play Room: {highlight_play_channel.mention}") if created else reused_resources.append(f"Highlight Play Room: {highlight_play_channel.mention}")
